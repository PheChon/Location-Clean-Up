...
DATA   = Path("/mnt/user-data/uploads/Location_TH_cleanup_new.xlsx")
MASTER = Path("/mnt/user-data/uploads/TH44_Addresses_Cleaned_V2.xlsx")
OUT    = Path("/mnt/user-data/outputs/SAP_Master_Stage2_Matched.xlsx")
...
def decide(row):
    ...
    for cus in sorted(org_targets):       # sorted() กัน set iteration order สุ่มข้ามรัน (reproducibility)
    ...
    ranked=sorted(candidates.items(), key=lambda kv:(-score_of(kv[1]), kv[0]))  # secondary key=cuscode กันคะแนนเท่ากันสุ่มข้ามรัน
    ...
    # แท็กเหตุผลก่อน tie-break (ใช้สำหรับ convergence check ภายหลัง เฉพาะเคส 'ชื่อกลาง+ที่อยู่ไม่ยืนยัน')
    is_moderate_name_case = (tier!='AUTO' and not (top['org'] and org_conflict)
                              and (top['name_score'] or 0)>=NAME_FLOOR and not top['generic']
                              and (top['addr_score'] or 0)<0.6)

    if len(ranked)>1 and tier=='AUTO':
        gap=top_score-score_of(ranked[1][1])
        if gap<15 and ranked[1][0]!=top_cus:
            tier='REVIEW'   # candidate สูสีกันเกินไป ไม่ชัวร์พอ auto (ใช้กับทุกกรณีเท่ากัน รวม org_sibling ด้วย — org อาจเป็น org ถังขยะ/master มี Cuscode ซ้ำได้)

    # ADDR-TIEBREAK: กู้เคสที่ถูกปัดตกเป็น REVIEW เพราะมีหลาย candidate (เช่น generic name / org_conflict)
    # แต่ที่อยู่ตัดความกำกวมได้ชัด — top candidate เป็น "ตำบลตรงเป๊ะ (addr=1.0) เพียงตัวเดียว" และมีชื่อพอเชื่อได้
    # ใช้เฉพาะ addr=1.0 (ระดับตำบล) เท่านั้น ไม่ใช้ 0.6 (อำเภอ) เพราะบริษัทใหญ่มักมีหลายสาขาในอำเภอเดียวกัน
    addr_tiebreak_applied=False
    if tier=='REVIEW' and (top['addr_score'] or 0)>=1.0 and (top['name_score'] or 0)>=NAME_FLOOR:
        n_top_addr = sum(1 for _,c in ranked if (c['addr_score'] or 0)>=1.0)
        if n_top_addr==1:   # มีตำบลตรงเป๊ะตัวเดียว = ที่อยู่ชี้ชัดว่าเป็นตัวนี้
            tier='AUTO'; addr_tiebreak_applied=True
    ...
    if addr_tiebreak_applied: methods.append('addr_tiebreak(tambon)')
...
# ══════════════════════════════════════════════════════════════════════════════
# PART E.1b — LIKELY_BRANCH rule: ชื่อ fuzzy กลาง + ที่อยู่ไม่ยืนยัน (REVIEW) แต่มีหลายแถวอื่น
# ชี้ไป Cuscode เดียวกันเป็นหลักฐานยืนยัน (น่าจะเป็นบริษัทเดียวกัน คนละสาขา)
# เกณฑ์ (คิดจากข้อมูลจริง — นับจำนวนอย่างเดียวไม่พอ ต้องมีคะแนนขั้นต่ำด้วย กันคะแนนบังเอิญเท่ากันหลายแถว):
#   ต้องมี >=3 แถวชี้ Cuscode เดียวกัน "และ" คะแนนชื่อของทุกแถวในกลุ่มต้อง >=0.65
# ไม่ถึงเกณฑ์: ยังเป็น REVIEW เหมือนเดิม แต่หมายเหตุไว้ว่าอาจเป็นสาขา (โอกาสต่ำ)
# ══════════════════════════════════════════════════════════════════════════════
print("Applying LIKELY_BRANCH rule (convergence >=3 + min name_score >=0.65) …")
BRANCH_MIN_COUNT, BRANCH_MIN_SCORE = 3, 0.65
moderate_rows = {i: v['moderate_info'] for i,v in match_rows.items() if v.get('moderate_info')}
target_rows = defaultdict(list)   # cuscode -> [row indices]
for i,(cus,sc) in moderate_rows.items(): target_rows[cus].append(i)

n_branch=0; n_noted=0
for cus,idxs in target_rows.items():
    scores=[moderate_rows[i][1] for i in idxs]
    qualifies = len(idxs)>=BRANCH_MIN_COUNT and min(scores)>=BRANCH_MIN_SCORE
    mr=mas_by_k[cus]
    if qualifies:
        for i in idxs:
            v=match_rows[i]
            v['action']='LIKELY_BRANCH'
            v['target']=cus
            v['method']=(v['method']+'+' if v['method'] else '')+f'branch_convergence({len(idxs)})'
            v['review']=f"น่าจะเป็นสาขาของ {mr['NAME1']} (Cuscode {cus.lstrip('0') or '0'}) — พบ {len(idxs)} แถวอื่นที่ชื่อคล้ายกันชี้ไปเป้าหมายเดียวกัน"
            n_branch+=1
    else:
        for i in idxs:
            v=match_rows[i]
            v['review']=v['review']+f" | อาจจะเป็นสาขาของ {mr['NAME1']} แต่เป็นไปได้ต่ำ (ชี้ซ้ำแค่ {len(idxs)} แถว)"
            n_noted+=1
print(f"  LIKELY_BRANCH: {n_branch:,} แถว | REVIEW (หมายเหตุ 'อาจเป็นสาขา แต่โอกาสต่ำ'): {n_noted:,} แถว")
print("  action distribution (หลัง LIKELY_BRANCH rule):", Counter(v['action'] for v in match_rows.values()))
...
# ไม่แตะ LIKELY_BRANCH (เจอเบาะแสบริษัทแม่แล้ว แม้ไม่มี WO/IB ก็ไม่ควรลบ เพราะมีเบาะแสให้ตามต่อ)
# ══════════════════════════════════════════════════════════════════════════════
...
def maskey_addr_check(row, mr):
    """เทียบที่อยู่จริงของแถว (Stage1) กับที่อยู่ตาม master ของ Cuscode เดียวกัน — self-check ว่า sap code ที่ตรงกันนี้ น่าจะถูกที่จริงไหม"""
    tid,did,pid = row['sap_tambon_id'], row['sap_district_id'], row['sap_province_id']
    mtid,mdid,mpid = mr.get('addr_tambon_id'), mr.get('addr_district_id'), mr.get('addr_province_id')
    if not any([tid,did,pid]) or not any([mtid,mdid,mpid]):
        return "NO_DATA (เทียบไม่ได้ — ไม่มีข้อมูลที่อยู่พอ)"
    if tid and mtid and tid==mtid: return "MATCH (ตำบลตรง)"
    if did and mdid and did==mdid: return "PARTIAL (อำเภอตรง ตำบลต่าง)"
    if pid and mpid and pid==mpid: return "WEAK (จังหวัดตรงอย่างเดียว)"
    return "MISMATCH (คนละจังหวัด)"
...
for i in sap.index[sap['maskey_status']=='MASKEY']:
    ...
    achk=maskey_addr_check(row, mr)
    is_name_bad = nchk!="OK"; is_addr_bad = achk.startswith("MISMATCH")
    if is_name_bad and is_addr_bad:
        rflag="⚠️ BOTH_MISMATCH — ชื่อและที่อยู่ไม่ตรงทั้งคู่ ควรตรวจด่วนว่า sap code นี้ผูกถูก location ไหม"
    elif is_name_bad: rflag="name_mismatch_only"
    elif is_addr_bad: rflag="addr_mismatch_only"
    else: rflag=""
    ...
    mk_info[i]=dict(cuscode=mr['Cuscode'], name=mr['NAME1'], fth=mr['addr_full_th'], fen=mr['addr_full_en'],
                     mstat=mr['addr_status'], nchk=nchk, achk=achk, rflag=rflag, multi=multi)
...
out['master_addr_status']=""; out['name_check']=""; out['maskey_addr_check']=""; out['maskey_review_flag']=""; out['multi_location_note']=""
for i,d in mk_info.items():
    ...
    out.at[i,'master_addr_status']=d['mstat']; out.at[i,'name_check']=d['nchk']
    out.at[i,'maskey_addr_check']=d['achk']; out.at[i,'maskey_review_flag']=d['rflag']
    out.at[i,'multi_location_note']=d['multi']
...
ADDED_MASKEY=['sap_match_status','master_cuscode','master_name','master_addr_full_th','master_addr_full_en',
              'master_addr_status','name_check','maskey_addr_check','maskey_review_flag','multi_location_note']
...
FILL_ACTION={"MASKEY":PatternFill("solid",start_color="B0BEC5"),"MERGE_WITH":PatternFill("solid",start_color="C8E6C9"),
      ...
      "LIKELY_BRANCH":PatternFill("solid",start_color="90CAF9")}
...
rflag_col=cols.index("maskey_review_flag")+1
...
for ri,row in enumerate(allrows,1):
    for ci,val in enumerate(row,1):
        ...
        if ri==1:
            ...
            elif ci==rflag_col and isinstance(val,str) and val.startswith("⚠️ BOTH_MISMATCH"): c.fill=RED
            elif ci==rflag_col and isinstance(val,str) and val.endswith("_mismatch_only"): c.fill=ORANGE
...
for k in ["MASKEY","MERGE_WITH","ASSIGN_CODE","LIKELY_BRANCH","REVIEW","NEW","DELETE"]:
...
rowsS+=[["รวม",N,"100%"],["",""],
    ...
    ["",""],
    ["MASKEY quality check (9,171 แถว — sap code ตรง master แต่ชื่อ/ที่อยู่จริงตรงไหม)","",""],
    ["  ⚠️ BOTH_MISMATCH (ชื่อและที่อยู่ไม่ตรงทั้งคู่ — ตรวจด่วน)",
     sum(1 for d in mk_info.values() if d['rflag'].startswith('⚠️')), "เสี่ยงสุด sap code อาจผูกผิด location"],
    ["  name_mismatch_only", sum(1 for d in mk_info.values() if d['rflag']=='name_mismatch_only'), "ชื่อไม่ตรง แต่ที่อยู่ยังพอเชื่อได้"],
    ["  addr_mismatch_only", sum(1 for d in mk_info.values() if d['rflag']=='addr_mismatch_only'), "ที่อยู่ไม่ตรง แต่ชื่อยังพอเชื่อได้"]]
...
for t,b in [("SAP ↔ Master — Stage 2: Multi-Signal Matching — วิธีการ",True),("",False),
 ...
 ("",False),("★ MASKEY self-check ใหม่: sap code ตรง master ไม่ได้แปลว่าชื่อ/ที่อยู่จะตรงเสมอ (อาจผูก code ผิด location ตั้งแต่ต้น)",True),
 ("  maskey_addr_check: เทียบที่อยู่จริงของแถว (Stage1) กับที่อยู่ตาม master ของ Cuscode เดียวกัน — MATCH/PARTIAL(อำเภอ)/WEAK(จังหวัด)/MISMATCH(คนละจังหวัด)/NO_DATA",False),
 ("  maskey_review_flag: รวมกับ name_check เดิม — '⚠️ BOTH_MISMATCH' (สีแดง) = ชื่อและที่อยู่ไม่ตรงทั้งคู่ ควรตรวจด่วนว่าผูก code ถูก location ไหม",False),
 ("  name_mismatch_only / addr_mismatch_only (สีส้ม) = ไม่ตรงแค่ด้านเดียว ยังพอเชื่อได้บ้าง ไม่เร่งด่วนเท่า",False),
 ...
 ("• candidate สูสีกันเกินไป (ต่างกัน<15 คะแนน) → REVIEW เสมอ ใช้กับทุกกรณีเท่ากัน รวม org_sibling ด้วย (แก้แล้ว — เดิมยกเว้น org_sibling แต่พบว่าหละหลวมเกินไป ดูหมายเหตุด้านล่าง)",False),
 ("• ADDR-TIEBREAK: ถ้าโดนปัดตกเป็น REVIEW เพราะ candidate สูสี แต่มี candidate 'ตำบลตรงเป๊ะ (addr=1.0) ตัวเดียว' + ชื่อพอเชื่อได้ (>=0.55) → กู้เป็น AUTO (ที่อยู่ตัดความกำกวมได้)",False),
 ("  ใช้เฉพาะ addr=1.0 (ตำบล) เท่านั้น ไม่ใช้ 0.6 (อำเภอ) เพราะบริษัทใหญ่มักมีหลายสาขาในอำเภอเดียวกัน แยก Cuscode ไม่ได้ด้วยอำเภอ",False),
 ("  ช่วยกู้ 87 แถว (จาก REVIEW 63 + DELETE 24 ที่เกือบถูกลบทั้งที่ระบุตัวได้) เช่น Toyota Boshoku, Cargill Meats, Nakhon Pathom Rajabhat",False),
 ...
 ("",False),("★★ ค้นพบสำคัญระหว่างตรวจ (แก้แล้ว): 'org ถังขยะ' กว้างกว่าที่คิดมาก ไม่ใช่แค่ 2 org",True),
 ("  เดิม exempt org_sibling (org เดียว ไม่ conflict) จาก tie-break เพราะเชื่อว่าแม่นสุด — ตรวจพบว่าหละหลวมเกินไป",False),
 ("  สุ่มตรวจ 20 แถวที่ org_sibling ชนะแบบ exempt พบว่า 20/20 เป็น org ถังขยะที่มี maskey เดียว (ไม่ conflict) ชี้ผิดบริษัทไปเลย",False),
 ("  เช่น Toyota/P&G/Bridgestone/PTT/Mahidol ถูกจะ merge เข้า Cuscode ของ HACH/AGILENT/CALIBRATION LAB ที่ไม่เกี่ยวกันเลย ทั้งที่ชื่อตัวเองตรงอยู่แล้ว",False),
 ("  แก้: ตัด exemption ออก ให้ tie-break (ต่างกัน<15 คะแนน→REVIEW) ใช้กับทุกกรณีเท่ากัน รวม org_sibling ด้วย",False),
 ("  ผล: MERGE_WITH ลด 479 แถว (ไปเป็น REVIEW 466 + DELETE 13) แต่ org_sibling ยังมีประโยชน์อยู่ 2,349 แถว (ไม่ conflict + ไม่มีคู่แข่งสูสี) ไม่ได้ไร้ค่า",False),
 ("",False),("★ Reproducibility fix: sorted() candidate/org iteration กัน Python hash-randomization ทำผลลัพธ์ต่างกันข้ามการรัน (ตรวจแล้วรัน 2 รอบได้ผลเหมือนกันเป๊ะ)",True),
 ...
 ("",False),("★ LIKELY_BRANCH rule: ชื่อ fuzzy กลาง (0.55-0.84) + ที่อยู่ไม่ยืนยัน เดิมลง REVIEW เฉย ๆ",True),
 ("  ถ้ามี >=3 แถวอื่นชี้ Cuscode เดียวกัน 'และ' ทุกแถวคะแนนชื่อ >=0.65 -> ยกเป็น LIKELY_BRANCH (สีฟ้า) พร้อมหมายเหตุชื่อบริษัทแม่",False),
 ("  เกณฑ์คิดจากข้อมูลจริง: นับจำนวนอย่างเดียวไม่พอ (พบกลุ่มที่ชี้ซ้ำ 4 ครั้งแต่ปลอมทั้งหมด คะแนนบังเอิญเท่ากัน) ต้องมีคะแนนขั้นต่ำด้วย",False),
 ("  ไม่ถึงเกณฑ์: ยัง REVIEW เหมือนเดิม แต่หมายเหตุว่า 'อาจจะเป็นสาขาของ X แต่เป็นไปได้ต่ำ'",False),
 ("  ตัวอย่างจริงที่จับได้: 37 location ของร้าน Ohkajhu สาขาต่าง ๆ -> ชี้ไป Cuscode เดียวกัน 'PLUK PHAK PRAW RAK MAE' (ชื่อจดทะเบียนจริง)",False),
